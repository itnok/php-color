...
class Color
{
    ...
    /**
     * Init color from HSV integer value
     *
     * Hue         (H) Integer 0-360
     ...
     * @return Color
     */
    public function fromHsvInt($h, $s, $v)
    {
        if($h < 0)   $h = 0;   // Hue:
        if($h > 360) $h = 360; //   0-360
        if($s < 0)   $s = 0;   // Saturation:
        if($s > 100) $s = 100; //   0-100
        if($v < 0)   $v = 0;   // Lightness:
        if($v > 100) $v = 100; //   0-100
        ...
        return $this->fromRgbInt( round( $dR ), round( $dG ), round( $dB ) );
    }
